...
<script lang="ts">
    ...
    const form = superForm(defaults(zod(appearanceFormSchema)), {
        SPA: true,
        validators: zod(appearanceFormSchema),
        async onUpdated(event) {
            if (event.form.valid) {
                console.log('submitted settings form', event.form.data);
                const {theme} = event.form.data;
                setMode(theme);
            }
        }
    });
